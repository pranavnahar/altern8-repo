...
import { IconApiApp, IconLayoutDashboard, IconLogs, IconMoneybag, IconReceipt, IconReport, IconSettings, IconUserCircle, IconUsers, IconWallet } from "@tabler/icons-react";
        ...
        <div className="flex flex-col flex-1 overflow-x-hidden overflow-y-auto">
          ...
          <div className="flex flex-col gap-2 mt-8">
            {loading ? ( Array(6).fill(0).map((_, idx) => (
              <Skeleton key={idx} className="h-8 w-full rounded-lg mb-1" />
            ))
            ) : (
              links.map((link, idx) => (
                <SidebarLink
