...
const Page = () => {
  ...
  const [referralLinkState, getReferralLinkAction] = useFormState(getReferralLink, { referralLink: '' })
  ...
  return (
    <div className="min-h-screen mt-10 text-xl font-semibold text-center">
      <div className="w-4/5 mx-auto">
        <h1 className="py-5 text-3xl text-white font-medium tracking-tight text-center">Invite New User</h1>
        <div className="flex-1 w-3/5 mx-auto pt-12">
          <Snippet symbol={false} variant="solid" color='primary' className='bg-white/10 text-white w-full rounded-lg text-sm px-5'>
          ...
          <form action={inviteAction} className='flex gap-5'>
            <input
              ...
              required
            />
            <Button variant="expandIcon" size="sm" iconPlacement="right" Icon={IconChevronRight} type="submit" className="text-sm max-w-max">
              Send Invite
            </Button>
          </form>
        </div>
        ...
        <div className="flex w-3/5 gap-5 mx-auto pt-12">
          ...
          <label htmlFor="upload-excel-file">
            <Button variant="expandIcon" size="sm" iconPlacement="right" Icon={IconUpload} type="button" className="text-sm max-w-max h-full">
              Upload Bulk Invites
            </Button>
          </label>
        </div>
